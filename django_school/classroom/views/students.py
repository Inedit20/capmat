...
from ..forms import Subscribe,actuelle,collab, Free, UpdateProjetForm, CommentForm_capteurs, StudentSignUpForm,ProfileCUpdateForm, CapteurForm, EventForm,ProjetForm, EquipeForm, UserUpdateForm, ProfileUpdateForm ,TypeCapteurForm, DomaineForm, SubjectForm, StudentInterestsForm, CommentForm
from ..models import Capteur,Question, Answer,Chaine,Collaboration_connaissances, Comment_display, Collaboration_actuelle,Equipe,favoris_expert, Quiz,Forum, Student,ExpertListe, TakenQuiz, User, Subject, Profile,Comment, Domaine, TypeCapteur
...
@login_required
def profile(request):
    comments = Quiz.objects.order_by("created_at")[:5]
...
def profile_view(request):
    ...
    return render(request, 'Profile/display_profile.html', context)
...
class HomeView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['comments'] = Quiz.objects.order_by("created_at").distinct()[:5]
        context['cities'] = Quiz.objects.values('subject').annotate(count=Count('subject')).order_by('-count')[:6]
        context['Chaines'] = Subject.objects.all()
...
class ExpertListView(ListView):
    ...
    def get_queryset(self):
        ...
        projet_interests = self.quiz.subject.values_list('pk', flat=True)
        projet_chaine= self.quiz.chaine.values_list('pk', flat=True)
        queryset = Student.objects.filter(chaine__in = projet_chaine, interests__in=projet_interests).distinct()
        return queryset
    ...
    def get_context_data(self, **kwargs):
        ...
        projet_interests = self.quiz.subject.values_list('pk', flat=True)
        projet_chaine= self.quiz.chaine.values_list('pk', flat=True)
        inter =  get_object_or_404(Quiz, pk=self.kwargs['pk'])
        context['projet'] = self.quiz
        context['mots_cles'] = Student.objects.filter(interests__in=projet_interests).distinct()
        context['chaine'] = Student.objects.filter(chaine__in = projet_chaine).distinct()
        context['list_chaine']=Chaine.objects.all()
        collaboration = Collaboration_connaissances.objects.filter(projet= inter).distinct()
        context['x']= 0
        context['chaine_conn'] = collaboration
        quiz = get_object_or_404(Quiz, pk=self.kwargs['pk'])
        student_interests = self.quiz.subject.values_list('pk', flat=True)
        student_domaine = self.quiz.domaine.values_list('pk', flat=True)
        context['subject'] = Student.objects.filter( interests__in=student_interests).distinct() 
        context['domaine'] = Student.objects.filter( domaine__in=student_domaine).distinct()       
        return context
...
#CollaborationRecherche des experts
@method_decorator([login_required, student_required], name='dispatch')
class ChaineListView(ListView):
    model = Quiz
    ordering = ('name', )
    context_object_name = 'quizzes'
    template_name = 'Projet/chaine_connaissance.html'
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(ChaineListView, self).get_context_data(**kwargs)
        # Add in the publisher
        projet_interests = self.quiz.subject.values_list('pk', flat=True)
        projet_chaine= self.quiz.chaine.values_list('pk', flat=True)
        inter =  get_object_or_404(Quiz, pk=self.kwargs['pk'])
        ...
        context['chaine'] = Student.objects.filter(chaine__in = projet_chaine).distinct()
        context['list_chaine']=Chaine.objects.all()
        collaboration = Collaboration_actuelle.objects.filter(projet= inter).distinct()
        context['x']= 0
        context['chaine_conn'] = collaboration
        return context
...
#   email = EmailMessage()
 ...
 #   with smtplib.SMTP(host='smtp.univ-lorraine.fr', port=587) as smtp:
 #     smtp.ehlo()
 #     smtp.starttls()
...
class MesProjetsView(ListView):
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(MesProjetsView, self).get_context_data(**kwargs)
        context['comments'] = Quiz.objects.order_by("created_at")[:5]
        return context
...
def display_detail(request, slug):
    ...
    posts = Quiz.objects.order_by("created_at")[:5]
    commentss = Quiz.objects.order_by("created_at").distinct()[:5]
    cities = Quiz.objects.values('subject').annotate(count=Count('subject')).order_by('-count')[:6]
    Chaines = Subject.objects.all()
    projet_interests = post.subject.values_list('pk', flat=True)
    projet_chaine= post.chaine.values_list('pk', flat=True)
    inter =  get_object_or_404(Quiz, slug=slug)
    projet = get_object_or_404(Quiz, slug=slug)
    mots_cles = Student.objects.filter(interests__in=projet_interests).distinct()
    chaine = Student.objects.filter(chaine__in = projet_chaine).distinct()
    list_chaine =Chaine.objects.all()
    collaboration = Collaboration_actuelle.objects.filter(projet= inter).distinct()
    x = 0
    chaine_conn = collaboration
    ...
    return render(request, template_name, {'post': post,
                                           'comments': comments,
                                           'posts' : posts,'commentss' : commentss,'cities' : cities,'Chaines' : Chaines,
                                            'projet_interests' :projet_interests,
                                            'projet_chaine': projet_chaine,
                                            'inter' :inter,
                                            'projet' :projet,
                                            'mots_cles' :mots_cles,
                                            'chaine':chaine,
                                            'list_chaine' :list_chaine,
                                            'x' :x,
                                            'chaine_conn' : collaboration,
...
@login_required
@student_required
def display_detail_public_coll(request, slug):
    template_name = 'Collabrations/display_public.html'
    post = get_object_or_404(Quiz, slug=slug)
    comments = Comment.objects.filter(post=post)
    posts = Quiz.objects.order_by("created_at")[:5]
    commentss = Quiz.objects.order_by("created_at").distinct()[:5]
...
@login_required
@student_required
def display_detail_public_coll_ac(request, slug):
    template_name = 'Collabrations/display_public_ac.html'
    post = get_object_or_404(Quiz, slug=slug)
    comments = Comment.objects.filter(post=post)
    posts = Quiz.objects.order_by("created_at")[:5]
    commentss = Quiz.objects.order_by("created_at").distinct()[:5]
...
def sub_projet(request, pk):
    ...
    if request.method == 'POST':
        ...
        html_content = '<p>Vous venez de recevoir une demande d&#8217;intégration de votre projet créer sur la Plateforme CAP-MAT.</p></br><p>Pour plus d&#8217;informations, Connectez-vous à votre compte <a href="http://erpiinnov.pythonanywhere.com" > CAP-MAT Plate-Forme<a> pour confirmer ou refuser la demande.</p></br><p>Bien à vous,</p></br><p><strong>Équipe CAP-MAT.</strong></p>'
...
def sub_creatcollab(request, pk_1, pk_2):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            post.projet= article
            post.porteur= request.user
            post.expert= expert.user
            post.save()
...
def confirmation_expert(request, pk_1, pk_2, pk_3, notice_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            html_content = '<p>Le développeur de concept viens de confirmer votre demande.</p></br><p>Pour plus d&#8217;informations, Connectez-vous à votre compte <a href="http://erpiinnov.pythonanywhere.com/" > CAP-MAT Plate-Forme<a> pour confirmer ou refuser la demande.</p></br><p>Bien à vous,</p></br><p><strong>Équipe CAP-MAT.</strong></p>'
...
def confirmation_copie(request, pk_1, pk_2, pk_3, notice_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = 'CAP-MAT Plate-Forme '
            text_content = porteur.email
            html_content = '<p>Le porteur de projet viens de confirmer votre demande.</p></br><p>Pour plus d&#8217;informations, Connectez-vous à votre compte <a href="http://erpiinnov.pythonanywhere.com/" > CAP-MAT Plate-Forme<a> pour confirmer ou refuser la demande.</p></br><p>Bien à vous,</p></br><p><strong>Équipe CAP-MAT.</strong></p>'
            recepient = str(porteur.email)
            msg = EmailMultiAlternatives(subject, text_content, EMAIL_HOST_USER, [recepient])
            msg.attach_alternative(html_content, "text/html")
            msg.send()
...
class expert_annuaire(ListView):
    context_object_name = 'expert'
    template_name = 'classroom/students/user/info_expert.html'


    def get_queryset(self):
        self.expert = get_object_or_404(User, pk=self.kwargs['pk_1'])
        return Student.objects.filter(pk = self.expert.student.pk)
...
def AnnuaireListView(request):
    exper_list = Student.objects.all()
...
class PdfGestionView (CreateView) :
    model = Quiz
    template_name = 'Pdfs/pdf.html'
